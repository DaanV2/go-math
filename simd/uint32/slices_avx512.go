...
func (s *Slice) AddToAll(v uint32) {
	...
	vec := archsimd.BroadcastUint32x16(v)

	l := vec.Len()
	var i int

	for i < (len(s.data) - l) {
		v := archsimd.LoadUint32x16(s.data[i : i+l])
		v = v.Add(vec)
...
func (s *Slice) Add(v []uint32) {
	...
	for i < (n - l) {
		a := archsimd.LoadUint32x16(s.data[i : i+l])
		b := archsimd.LoadUint32x16(v[i : i+l])
		a = a.Add(b)
...
func (s *Slice) Sum() uint32 {
	...
	l := 16
	var i int
	acc := archsimd.BroadcastUint32x16(0)
...
func (s *Slice) Min() uint32 {
	...
	if len(s.data) >= l {
		acc := archsimd.LoadUint32x16(s.data[0:l])
		i = l
		for i < (len(s.data) - l) {
			v := archsimd.LoadUint32x16(s.data[i : i+l])
			acc = acc.Min(v)
