...
func (s *Slice) AddToAll(v int64) {
	...
	vec := archsimd.BroadcastInt64x8(v)

	l := vec.Len()
	var i int

	for i < (len(s.data) - l) {
		v := archsimd.LoadInt64x8(s.data[i : i+l])
		v = v.Add(vec)
...
func (s *Slice) Add(v []int64) {
	...
	for i < (n - l) {
		a := archsimd.LoadInt64x8(s.data[i : i+l])
		b := archsimd.LoadInt64x8(v[i : i+l])
		a = a.Add(b)
...
func (s *Slice) Min() int64 {
	...
	if len(s.data) >= l {
		acc := archsimd.LoadInt64x8(s.data[0:l])
		i = l
		for i < (len(s.data) - l) {
			v := archsimd.LoadInt64x8(s.data[i : i+l])
			acc = acc.Min(v)
