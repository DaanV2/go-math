...
// NOTE: this file uses the local Int64x4 wrapper (defined alongside this
// package) rather than archsimd.Int64x4 directly, since native
// Abs/Mul/Min/Max/ShiftRight on 64-bit lanes require AVX-512 instructions
// (VPABSQ/VPMULLQ/VPMINSQ/VPMAXSQ/VPSRAQ) that don't exist on plain AVX2
// hardware. The wrapper emulates them with AVX2-safe sequences.
...
func (s *Slice) AddToAll(v int64) {
	...
	vec := NewInt64x4Boardcast(v)

	l := vec.Len()
	var i int

	for i < (len(s.data) - l) {
		v := NewInt64x4(s.data[i : i+l])
		v = v.Add(vec)
...
// performs: s[i] = s[i] >> count
// NOTE: 64-bit arithmetic right shift has no AVX2 instruction, so this uses
// the emulated [Int64x4.ShiftRight].
...
func (s *Slice) Add(v []int64) {
	...
	for i < (n - l) {
		a := NewInt64x4(s.data[i : i+l])
		b := NewInt64x4(v[i : i+l])
		a = a.Add(b)
