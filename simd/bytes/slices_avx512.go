...
func (s *Slice) AddToAll(v byte) {
	...
	vec := archsimd.BroadcastUint8x64(v)

	l := vec.Len()
	var i int

	for i < (len(s.data) - l) {
		v := archsimd.LoadUint8x64(s.data[i : i+l])
		v = v.Add(vec)
...
func (s *Slice) Add(v []byte) {
	...
	for i < (n - l) {
		a := archsimd.LoadUint8x64(s.data[i : i+l])
		b := archsimd.LoadUint8x64(v[i : i+l])
		a = a.Add(b)
...
func (s *Slice) Min() byte {
	...
	if len(s.data) >= l {
		acc := archsimd.LoadUint8x64(s.data[0:l])
		i = l
		for i < (len(s.data) - l) {
			v := archsimd.LoadUint8x64(s.data[i : i+l])
			acc = acc.Min(v)
