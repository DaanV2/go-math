...
func Test_Float32x4_Ops(t *testing.T) {
	...
	vec1 := simdfloats.NewFloat32x4(dataA)
	vec2 := simdfloats.NewFloat32x4(dataB)
	vec3 := simdfloats.NewFloat32x4(dataC)
	...
	t.Run("Abs", func(t *testing.T) {
		vec := simdfloats.NewFloat32x4([]float32{1, -2, -3, 4})
		result := vec.Abs()
	...
	t.Run("Scale", func(t *testing.T) {
		exponents := simdfloats.NewFloat32x4([]float32{1, 0, 2, 1})
...
func Test_Float32x4_Load(t *testing.T) {
	...
	assert.Equal(t, simdfloats.NewFloat32x4(data).ToSlice(), data[:4])
	assert.Equal(t, []float32{1, 0, 0, 0}, simdfloats.NewFloat32x4(data[:1]).ToSlice())
	assert.Equal(t, []float32{1, 2, 3, 0}, simdfloats.NewFloat32x4(data[:3]).ToSlice())
}
...
func Test_Float32x4Slice(t *testing.T) {
	...
	result := simdfloats.NewFloat32x4Slice(data)
...
func ExampleFloat32x4() {
	...
	v1 := simdfloats.NewFloat32x4(data)
